...
public class CredentialsValidatorTest {
    ...
    private void initValidator(final @NotNull String credentials, final boolean hashed) throws Exception {
    ...
    /**
     * Credentials where user3 references "role_that_does_not_exist", which has no
     * corresponding entry in the roles section. This reproduces the NPE bug in
     * CredentialsValidator#getPermissions when roles.get(clientRole) returns null.
     */
    private static final String CREDENTIALS_WITH_UNKNOWN_ROLE = "<file-rbac>" +
            "   <users>\n" +
            "        <user>\n" +
            "            <name>user3</name>\n" +
            "            <password>pass3</password>\n" +
            "            <roles>\n" +
            "                <id>role_that_does_not_exist</id>\n" +
            "            </roles>\n" +
            "        </user>\n" +
            "    </users>\n" +
            "    <roles>\n" +
        ...
        /**
     * Regression test for: NullPointerException in CredentialsValidator#getPermissions
     * when a user's role does not exist in the roles configuration.
     *
     * Before the fix, roles.get(clientRole) returned null and the subsequent call to
     * role.getPermissions() threw a NullPointerException.
     * After the fix, unknown roles are skipped and an empty list is returned safely.
     */
    @Test
    public void test_permissions_with_unknown_role() throws Exception {
        initValidator(CREDENTIALS_WITH_UNKNOWN_ROLE, false);

        // Should not throw NullPointerException — unknown role must be skipped gracefully
        final List<TopicPermission> permissions = validator.getPermissions(
                "client3", "user3", List.of("role_that_does_not_exist"));

        assertNotNull("Expected a non-null result even when role is unknown", permissions);
        assertTrue("Expected empty permissions when all roles are unknown", permissions.isEmpty());
    }
}
